...
import { setQuery, clearSearch, fetchProducts } from "@/features/search/searchSlice";
import { menuFetch } from "@/api/menu";
import { getGlobalSettings } from "@/api/globalSettings";

export default function Header() {
  const dispatch = useDispatch();
  const { query, results, loading, showResults } = useSelector((state) => state.search);
  ...
  useEffect(() => {
    const debounce = setTimeout(() => {
      if (query.trim().length > 1) dispatch(fetchProducts(query));
    }, 400);
          ...
          <ul className="hidden md:flex space-x-8 font-semibold uppercase text-sm relative">
            {menu.map((item) => (
              <li key={item.id} className="group relative">
                <Link href={item.slug} className="hover:text-secondary transition-all duration-300 flex items-center gap-1">
                  {item.name}
                  {item.children?.length > 0 && <span className="mt-1 text-xs">▼</span>}
                </Link>

                {item.children?.length > 0 && (
                  <ul className="absolute left-0 top-full hidden group-hover:flex flex-col bg-base-white dark:bg-base-black dark:text-base-white shadow-xl rounded-xl py-2 min-w-[200px] transform scale-95 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-300 ease-out z-50">
                    {item.children.map((child) => (
                      <li key={child.id}>
                        <Link href={child.slug} className="block px-5 py-2.5 rounded-md hover:bg-black hover:text-white transition-colors duration-300">
                          {child.name}
                        </Link>
                      </li>
                    ))}
          ...
          <div className="hidden md:flex items-center relative">
            <div className="flex items-center border-2 border-neutral-light/30 rounded-full px-4 py-2.5 w-64 bg-neutral-light/5 hover:border-primary/50 transition-all duration-200 shadow-sm">
              <input
                ...
                onFocus={() => query.trim().length > 1 && dispatch(setQuery(query))}
                onBlur={() => setTimeout(() => dispatch(clearSearch()), 200)}
                placeholder="Search products..."
                className="flex-grow px-3 py-1 focus:outline-none bg-transparent placeholder:text-neutral-light/70 font-medium"
              />
              <Search className="text-neutral-light hover:text-primary cursor-pointer transition-colors duration-200" size={22} />
            </div>
                    ...
                    {results.map((product) => (
                      ...
                      >
                        ...
                        <div className="flex-grow">
                          <h4 className="font-semibold text-sm text-gray-900 dark:text-white line-clamp-1">{product.title || product.name}</h4>
                          {product.price && <p className="text-xs font-bold text-green-600 mt-1">RS: {product.price.toLocaleString()}</p>}
                        </div>
                      </Link>
                    ))}
                  </div>
